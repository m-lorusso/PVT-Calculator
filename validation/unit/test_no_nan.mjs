...
const CHECK_PATHS = [
  path.join(VALIDATION_ROOT, "reference", "reference_summary.json"),
  path.join(VALIDATION_ROOT, "reference", "deep_results.json"),
  path.join(VALIDATION_ROOT, "fixtures", "backend", "backend_sydney.json"),
  path.join(VALIDATION_ROOT, "fixtures", "backend", "backend_melbourne.json"),
  path.join(VALIDATION_ROOT, "fixtures", "backend", "backend_perth.json"),
  path.join(VALIDATION_ROOT, "fixtures", "weather"),
];
...
for (const file of files){
  ...
  ok(`${path.relative(VALIDATION_ROOT, file)} has finite JSON values`, problems.length === 0, problems.slice(0, 3).join("; "));
}
